...
const elements = {
    ...
    educationLength: () => document.getElementById('educationLength')?.value,
...
async function waitForAgents() {
    let retries = 0;
    const requiredAgents = ['BachelorMatchAgent', 'BachelorSchoolAgent', 'BachelorCompareAgent', 'BachelorStatementAgent', 'FeedbackAgent'];
    
    while (retries < 50) {
        const allReady = requiredAgents.every(agent => window[agent]);
        if (allReady) {
            console.log('✅ 所有本科 Agent 已就绪');
            return true;
        }
...
async function startMatching() {
    const userData = {
        ...
        educationLength: elements.educationLength(),
        gpa: elements.gpa(),
        highSchoolType: elements.highSchoolType(),
        courseList: elements.courseList(),
        achievements: elements.achievements(),
        useType: elements.useType()
    };
    
    const required = ['userName', 'universityName', 'overseasMajor', 'courseList'];
    ...
    window.Tracker?.startSession({ ...userData, level: 'bachelor' });
    window.Tracker?.trackEvent('start_matching', { level: 'bachelor' });
    
    showLoading();
    
    try {
        updateMatchResult('正在匹配专业...');
        const matches = await window.BachelorMatchAgent.match(userData);
...
async function recommendSchools() {
    ...
    try {
        const schools = await window.BachelorSchoolAgent.recommend(
...
async function generateCompareTable() {
    ...
    try {
        ...
        const rows = await window.BachelorCompareAgent.generate(
...
async function generateStatement() {
    ...
    try {
        const result = await window.BachelorStatementAgent.generate({
            userName: elements.userName(),
            universityName: elements.universityName(),
            overseasMajor: elements.overseasMajor(),
            specialization: elements.specialization(),
...
function renderMatchResult(matches) {
    if (!matches.length) {
        updateMatchResult('<p>未匹配到合适专业，请补充更多课程信息后重试</p>');
        return;
    }
        ...
        html += `
            <div class="discipline-card ${idx === 0 ? 'selected' : ''}" onclick="window.selectDiscipline(${idx})">
                ...
                <div class="discipline-meta">
                    <span class="first-level">${escapeHtml(m.category || '')}</span>
        ...
        html += `
            <div class="school-card">
                ...
                <div class="school-link">
                    ${isDirectLink 
                        ? `🔗 <a href="${escapeHtml(s.url)}" target="_blank">查看培养方案</a>`
...
function renderTableResult(rows) {
    const tableHtml = window.BachelorCompareAgent.toHtml(rows);
...
window.downloadStatement = function() {
    ...
    link.download = `本科复核自述_${elements.userName() || '申请人'}.txt`;
    link.click();
...
window.downloadCSV = function() {
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `本科对比表_${new Date().toISOString().slice(0, 19)}.csv`;
    link.click();
    URL.revokeObjectURL(link.href);
};
